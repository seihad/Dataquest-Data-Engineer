...
class BTree:
    ...
    def __find_node(self, current_node, key):
        if current_node.contains_key(key):
            return current_node
        if current_node.is_leaf():
            return None
        child_index = current_node.get_insert_index(key) 
        return self.__find_node(current_node.children[child_index], key)
    ...
    def __add(self, current_node, key, value):
        if current_node.is_leaf(): 
            current_node.insert_entry(key, value) 
        else:
            child_index = current_node.get_insert_index(key) 
            self.__add(current_node.children[child_index], key, value)
    ...
    def add(self, key, value):
        self.__add(self.root, key, value)
        self.size += 1
